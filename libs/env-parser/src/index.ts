export function createEnvReader(envs: { [key: string]: string | undefined }) {
  function toInt(variable: string, value: string): number {
    ...
    if (!Number.isFinite(int)) {
      throw new Error(`Expected integer for environment env: ${variable} but got ${value}`);
    }
    return int;
  }
  ...
  function toBool(variable: string, value: string): boolean {
    switch (value.toLowerCase()) {
      ...
      case '0':
        return false;
      default:
        throw new Error(`Expected boolean for environment env: ${variable} but got ${value}`);
    }
  }
